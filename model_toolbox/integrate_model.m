...
for t=1:NTIM_CHEM-1   %since we integrate to t = t+1, loop to NTIM_CHEM-1
    ...
    if (run_vert_diff == 1) %if doing the vertical diffusion, do this
        ...
        for counter=1:1:diff_rep
            ...
            %Read in measured ozone concentrations (convert from ppb to
            %molec cm-3)
            [airconc(:)] = getAirConc(temperature, press);
            [o3_measured(:)]=(obs_o3_interp(t+1)*airconc(1))/1E9;
            
            %calculate surface source for chlorine and bromine
            if ((run_chem == 1) && (add_surface_source_Cl_Br == 1))
                [spec(:,:,t+1),surface_source] = surface_source_cl_br(spec(:,:,t+1), dt_diff, BOX_WALL*100, temp_depo(:,1), o3_measured(:));
                
                %total surface source emissions sum, on model levels for each species
                surface_source_emissions.Cl2_total_surf_source(:,:,t+1)=surface_source_emissions.Cl2_total_surf_source(:,:,t+1)+surface_source.total_surf_source_t(1,:);
                surface_source_emissions.Cl2_photo_surf_source(:,:,t+1)=surface_source_emissions.Cl2_photo_surf_source(:,:,t+1)+surface_source.photo_surf_source_t(1,:);
                surface_source_emissions.Cl2_clono2_surf_source(:,:,t+1)=surface_source_emissions.Cl2_clono2_surf_source(:,:,t+1)+surface_source.clono2_surf_reactions_t(:,:);
                surface_source_emissions.Cl2_hocl_surf_source(:,:,t+1)=surface_source_emissions.Cl2_hocl_surf_source(:,:,t+1)+surface_source.hocl_surf_reactions_t(:,:);
                surface_source_emissions.Br2_total_surf_source(:,:,t+1)=surface_source_emissions.Br2_total_surf_source(:,:,t+1)+surface_source.total_surf_source_t(2,:);
                surface_source_emissions.Br2_photo_surf_source(:,:,t+1)=surface_source_emissions.Br2_photo_surf_source(:,:,t+1)+surface_source.photo_surf_source_t(2,:);
                surface_source_emissions.Br2_brono2_surf_source(:,:,t+1)=surface_source_emissions.Br2_brono2_surf_source(:,:,t+1)+surface_source.brono2_surf_reactions_t(:,:);
                surface_source_emissions.Br2_hobr_surf_source(:,:,t+1)=surface_source_emissions.Br2_hobr_surf_source(:,:,t+1)+surface_source.hobr_surf_reactions_t(:,:);
            end
        ...
        if (add_surface_source_Cl_Br == 1)
            surface_source_emissions.Cl2_total_surf_source(:,:,t+1)  =  surface_source_emissions.Cl2_total_surf_source(:,:,t+1)/double(diff_rep);
            surface_source_emissions.Cl2_photo_surf_source(:,:,t+1)  =  surface_source_emissions.Cl2_photo_surf_source(:,:,t+1)/double(diff_rep);
            surface_source_emissions.Cl2_clono2_surf_source(:,:,t+1) =  surface_source_emissions.Cl2_clono2_surf_source(:,:,t+1)/double(diff_rep);
            surface_source_emissions.Cl2_hocl_surf_source(:,:,t+1)   =  surface_source_emissions.Cl2_hocl_surf_source(:,:,t+1)/double(diff_rep);
            surface_source_emissions.Br2_total_surf_source(:,:,t+1)  =  surface_source_emissions.Br2_total_surf_source(:,:,t+1)/double(diff_rep);
            surface_source_emissions.Br2_photo_surf_source(:,:,t+1)  =  surface_source_emissions.Br2_photo_surf_source(:,:,t+1)/double(diff_rep);
            surface_source_emissions.Br2_brono2_surf_source(:,:,t+1) =  surface_source_emissions.Br2_brono2_surf_source(:,:,t+1)/double(diff_rep);
            surface_source_emissions.Br2_hobr_surf_source(:,:,t+1)   =  surface_source_emissions.Br2_hobr_surf_source(:,:,t+1)/double(diff_rep);
        end
